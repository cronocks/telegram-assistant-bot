...
Chuyển hệ thống từ **single-user** (chat_id allowlist hardcode) sang **multi-user** với SQLite-backed user registry, roles, parent-child relationships, per-user quota, birthdate flow, và password infra (chuẩn bị cho FR-5, chưa expose).
...
| 2.8 | Documentation update | ROADMAP Section 8 + Decision Log |
...
| 8 | `db/migrations/005_parent_links.sql` | `parent_links` |
| 9 | `db/migrations/006_quota.sql` | `user_quotas` |
| 10 | `db/migrations/007_password.sql` | Add password columns vào `users` |
| 11 | `user_store.py` | `SqliteUserStore` — CRUD users + bindings + invites |
| 12 | `permissions.py` | Role-based ACL helpers + decorators |
| 13 | `auth.py` | argon2id hash/verify + token generation |
| 14 | `text_utils.py` | `normalize_vn()` + prefix matcher |
| 15 | `tests/__init__.py` | Tests package init |
| 16 | `tests/conftest.py` | Pytest fixtures (in-memory SQLite, sample users) |
| 17 | `tests/test_user_store.py` | UserStore CRUD tests |
| 18 | `tests/test_permissions.py` | Role enforcement tests |
| 19 | `tests/test_auth.py` | Argon2 hash/verify tests |
| 20 | `tests/test_text_utils.py` | Normalize + matcher tests |
| 21 | `tests/test_cost_monitor.py` | Per-user quota tests |
| 22 | `requirements-dev.txt` | Dev dependencies (pytest, pytest-asyncio) |

### 4.2 Edited files

| # | File | Change |
|---|------|--------|
| 23 | `interfaces.py` | Thêm `User` dataclass, `UserStore` Protocol, `AuthClient` Protocol |
| 24 | `main.py` | DB init + migrations on startup; bootstrap admin; CoreDeps update |
| 25 | `core_handler.py` | Replace chat_id check với user lookup; thêm 10 commands mới; pass `User` xuống mọi `_cmd_*` handler |
| 26 | `channel_telegram.py` | Bỏ `is_authorized()` (logic chuyển sang core) |
| 27 | `cost_monitor.py` | Track theo `user_id`; check quota trước LLM call |
| 28 | `config.py` | Thêm `SQLITE_PATH` env var (default `./bot.db`) |
| 29 | `requirements.txt` | Thêm `argon2-cffi` |
| 30 | `docs/ROADMAP.md` | Update Section 8 status; add Decision Log entries phát sinh |

**Total: 30 file changes.**
...
- Schema migration `007_password.sql` thêm cột vào `users`
...
| # | Commit message (English) | Files |
|---|--------------------------|-------|
| 1 | `feat(db): SQLite infrastructure + migration runner` | `db/*`, `db/migrations/001_initial.sql`, `config.py`, `.gitignore` |
...
| 7 | `feat(family): parent_links + digest config commands` | `db/migrations/005_parent_links.sql`, `user_store.py`, `core_handler.py`, tests |
| 8 | `feat(quota): per-user quota tracking` | `db/migrations/006_quota.sql`, `cost_monitor.py`, `core_handler.py`, `tests/test_cost_monitor.py` |
| 9 | `feat(auth): argon2id password infrastructure (not yet exposed)` | `db/migrations/007_password.sql`, `auth.py`, `interfaces.py`, `requirements.txt`, `tests/test_auth.py` |
| 10 | `docs: update ROADMAP Section 8 + decision log entries from FR-2` | `docs/ROADMAP.md` |

Mỗi commit độc lập build-pass (import OK, migrations apply, có thể chạy `pytest`). FR-2 "hoàn chỉnh" sau commit 10.
...
| `text_utils` | normalize_vn cases (đ, ư, ơ, accents); match_command longest-first; empty input |
...
- [ ] Tất cả 10 commit landed trên `feature/FR2`
- [ ] `pytest` pass 100% (không skip, không xfail)
- [ ] Manual smoke test (Section 11.3) pass tất cả 9 bước
