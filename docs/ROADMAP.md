...
### 3.6 Phân vai trò trong git operations

| Việc | Người làm |
|------|-----------|
| Tạo feature branch (từ `main`) | **Claude** |
| Commit code | **Claude** |
| Push branch lên remote | **Claude** |
| Cung cấp link PR / diff để review | **Claude** |
| **Merge vào dev** (bằng git bash local) | **User** |
| **Merge vào main** (bằng git bash local) | **User** |
| **Xóa feature branch** (cả local lẫn remote, sau khi đã vào main) | **User** |
| Hỗ trợ khi có conflict / revert / hot-fix / sai workflow | **Claude** (chỉ khi user nhờ) |

**Lý do:** User giữ quyền kiểm soát tuyệt đối với production code. Claude không bao giờ tự ý merge hay xóa branch — kể cả khi feature đã được verify OK. Mọi thao tác "ghi vào main/dev" hoặc "xóa pointer" đều phải qua tay user.

Nếu trong lúc làm việc Claude phát hiện cần thao tác git destructive (reset, force push, rebase chia sẻ branch, ...) → **dừng lại, trình bày, đợi user xác nhận** (cùng quy tắc Phase 2 của CLAUDE.md).
