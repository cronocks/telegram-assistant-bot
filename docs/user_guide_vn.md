...
8. [Gia phả & Mộ phần](#8-gia-phả--mộ-phần)
9. [Hỏi đáp tự do](#9-hỏi-đáp-tự-do)
10. [Hồ sơ cá nhân](#10-hồ-sơ-cá-nhân)
...
2. Truy cập: `https://assistant-bot.onrender.com/login`
...
### Web UI
Trang chi tiêu trên Web UI (`/ledger`) cho phép xem danh sách bút toán, báo cáo tháng/năm, quản lý danh mục và hạn mức trực tiếp trên trình duyệt — không cần gõ lệnh Telegram.

---

## 8. Gia phả & Mộ phần

Lưu hồ sơ người thân (cả người đã mất), quan hệ gia phả và địa điểm chôn cất — tra cứu nhanh khi đi tảo mộ.

> Lệnh **xem/tra cứu** mở cho mọi thành viên. Lệnh **thêm/sửa/xóa** chỉ dành cho admin/manager (dữ liệu gia phả là tài sản chung của gia đình).

### Thêm hồ sơ người thân (admin/manager)
```
them nguoi than: Nguyễn Văn A, doi 3, sinh am 10/2/1920, mat am 15/7/1990
them nguoi than: Cụ Tổ, doi 1, sinh khoang 1850
them nguoi than: Bà B, gioi tinh nu, ten goi Bà Ngoại, ghi chu Quê Nam Định
```
> Ngày: `am DD/MM/YYYY` (âm lịch), `duong DD/MM/YYYY` (dương lịch), `YYYY` (chỉ biết năm), `khoang YYYY` (năm ước chừng)
> Các mục tùy chọn: `doi` (đời thứ mấy), `sinh`, `mat`, `gioi tinh nam/nu`, `ten goi` (tên thường gọi), `chi` (chi/nhánh họ), `ghi chu`

### Xem & tra cứu
```
danh sach nguoi than                — liệt kê tất cả
danh sach nguoi than doi 3          — lọc theo đời
xem nguoi than 5                    — hồ sơ đầy đủ #5 (kèm mộ phần nếu có)
xem nguoi than ong noi              — tìm theo tên/tên gọi (không cần dấu)
```

### Mộ phần
```
them mo phan: 5, Nghĩa trang Văn Điển, dia chi Thanh Trì Hà Nội, gps 20.9456,105.8231, lo B3 hàng 12, ghi chu Cạnh mộ cụ X
tim mo ong noi                      — tra cứu nhanh: địa chỉ + link Google Maps + vị trí lô
tim mo 5                            — tra cứu theo id
```
> Có `gps` thì kết quả kèm link Google Maps mở thẳng trên điện thoại.
> Thêm mộ phần mới cho người đã có mộ = ghi nhận cải táng/di dời (bản cũ giữ làm lịch sử).

### Sửa / xóa (admin/manager)
```
sua nguoi than: 5, doi=4, ten=Nguyễn Văn B
sua mo phan: 3, dia chi=Địa chỉ mới, gps 21.0,105.5
xoa mo phan: 3
