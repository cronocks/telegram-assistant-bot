...
| `family_tree.py` | `ancestors()`, `descendants()`, `family_roots()`, `render_tree()` — text tree for chat (spouses inline with ♥, children merged); `build_tree_structure()` — generation-row card tree for web (couple units, SVG connectors); both use recursive CTEs (FR-11) |
...
| `web_context.py` | Shared globals for all sub-routers: store references, token helpers, cookie helpers, `_resolve_user`, `init_web_router()` — populated once at startup (FR-5 → FR-11) |
| `web_router.py` | Thin aggregator: re-exports `init_web_router`; `include_router` for all sub-routers in order; `__getattr__` for backward-compat (FR-5 → FR-11) |
